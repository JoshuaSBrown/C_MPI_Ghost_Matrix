...
int main(void){
    ...
    printf("Testing: updateSouthGhostRowsGhostMatrix\n");
    {
        ...
        if(num_proc!=2){
          ...
          ghost_matrix * gmat = newGhostMatrixCheckerboard(2,2,0,2,0,0);
          ghost_matrix * gmat2 = newGhostMatrixCheckerboard(2,2,0,2,2,0);

          if(my_rank==0){
            setAllGhostMatrix(gmat,2.0);
            updateSouthGhostRowsGhostMatrix(gmat,my_rank,0,1,1);
            setAllGhostMatrix(gmat,3.0);
            setElemGhostMatrix(gmat,0,0,-1.0);
            updateSouthGhostRowsGhostMatrix(gmat,my_rank,0,1,2);
            setAllGhostMatrix(gmat2,4.5);
            updateSouthGhostRowsGhostMatrix(gmat2,my_rank,0,1,2);
          }else if(my_rank==1){
            updateSouthGhostRowsGhostMatrix(gmat,my_rank,0,1,1);
            // Should fill up the row closest to the core matrix first
            assert(getElemGhostMatrix(gmat,2,0)==2.0);
            assert(getElemGhostMatrix(gmat,2,1)==2.0);
            assert(getElemGhostMatrix(gmat,3,0)==0.0);
            assert(getElemGhostMatrix(gmat,3,1)==0.0);
            // Testing with 2 rows 
            updateSouthGhostRowsGhostMatrix(gmat,my_rank,0,1,2);
            printGhostMatrix(gmat);
            assert(getElemGhostMatrix(gmat,3,0)==3.0);
            assert(getElemGhostMatrix(gmat,3,1)==3.0);
            assert(getElemGhostMatrix(gmat,2,0)==-1.0);
            assert(getElemGhostMatrix(gmat,2,1)==3.0);
            // Test when ghost columns are also present
            updateSouthGhostRowsGhostMatrix(gmat2,my_rank,0,1,2);
            assert(getElemGhostMatrix(gmat2,3,2)==0.0);
            assert(getElemGhostMatrix(gmat2,3,3)==0.0);
            assert(getElemGhostMatrix(gmat2,2,2)==0.0);
            assert(getElemGhostMatrix(gmat2,2,3)==0.0);
